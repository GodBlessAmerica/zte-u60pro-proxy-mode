...
func saveModeJSON(w http.ResponseWriter, r *http.Request) {
	...
	dst := filepath.Join(configDir, fmt.Sprintf("mode%d.json", n))
...
<div class="panel"><h2>Modes</h2><div id="modebar" class="modebar"></div><div class="editor-head"><label>Mode <input id="modeNumber" type="number" min="1" max="99" value="5"></label><button id="saveMode">Validate & save</button><button id="saveStartMode">Save & activate</button></div><textarea id="modeJson" spellcheck="false" placeholder='Paste a complete sing-box JSON configuration here'></textarea><div id="modeMsg" class="msg">JSON is validated with sing-box before it can replace modeN.json. Existing files are backed up as modeN.json.bak-web.</div><div class="hint">Production credentials remain on this device only. The browser sends the JSON directly to 10.66.0.1; this feature does not upload configs to GitHub.</div></div>
...
async function saveMode(start){let n=parseInt($('modeNumber').value,10),body=$('modeJson').value.trim();if(!n||n<1||n>99){$('modeMsg').textContent='Mode number must be 1-99.';return}if(!body){$('modeMsg').textContent='Paste JSON first.';return}try{JSON.parse(body)}catch(e){$('modeMsg').textContent='Browser JSON check failed: '+e.message;return}try{$('modeMsg').textContent='Validating on U60…';let j=await post('/api/mode/save?number='+n,body);$('modeMsg').textContent=j.message;if(start)await activateMode(n);else await refreshModes()}catch(e){$('modeMsg').textContent='Save failed: '+e.message}}
async function activateMode(n){try{$('modeMsg').textContent='Activating mode '+n+'…';let j=await post('/api/mode/activate?number='+n);$('modeMsg').textContent=j.message;await refresh()}catch(e){$('modeMsg').textContent='Activate failed: '+e.message}}
document.querySelectorAll('[data-t]').forEach(b=>b.onclick=()=>traffic(b.dataset.t));$('refresh').onclick=refresh;$('saveMode').onclick=()=>saveMode(false);$('saveStartMode').onclick=()=>saveMode(true);refresh();setInterval(refresh,15000);
</script></body></html>`
...
func main() {
	...
	mux.HandleFunc("/api/mode/activate", func(w http.ResponseWriter, r *http.Request) {
		...
		if !tokenOK(r) { writeJSON(w, 403, apiReply{"ok": false, "error": "invalid token"}); return }
		n, err := parseModeNumber(r)
		if err != nil { writeJSON(w, 400, apiReply{"ok": false, "error": err.Error()}); return }
		out, err := run("restart", strconv.Itoa(n))
		if err != nil { writeJSON(w, 500, apiReply{"ok": false, "error": out}); return }
	...
	mux.HandleFunc("/api/traffic", func(w http.ResponseWriter, r *http.Request) {
		...
		if a != "on" && a != "off" && a != "selective" { writeJSON(w, 400, apiReply{"ok": false, "error": "invalid action"}); return }
	...
	mux.HandleFunc("/api/device", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost { writeJSON(w, 405, apiReply{"ok": false, "error": "POST required"}); return }
		if !tokenOK(r) { writeJSON(w, 403, apiReply{"ok": false, "error": "invalid token"}); return }
		m := strings.ToLower(r.URL.Query().Get("selector")); a := r.URL.Query().Get("action")
		if !macRE.MatchString(m) || (a != "proxy" && a != "direct") { writeJSON(w, 400, apiReply{"ok": false, "error": "invalid selector/action"}); return }

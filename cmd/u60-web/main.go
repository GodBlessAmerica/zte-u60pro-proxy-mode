...
*{box-sizing:border-box}body{margin:0;background:#0b0f14;color:#edf2f7;font-family:system-ui,-apple-system,Segoe UI,sans-serif}.wrap{max-width:980px;margin:auto;padding:26px 18px 50px}header{display:flex;justify-content:space-between;align-items:flex-start;gap:16px}h1{margin:3px 0 4px;font-size:30px}.sub{color:#8ea0b5;font-size:13px}.badge,.chip{display:inline-flex;align-items:center;border-radius:999px;padding:6px 10px;font-size:12px;background:#1a2230;color:#cbd5e1}.ok{background:#12351f;color:#9be7b0}.warn{background:#3b2e12;color:#f4d27a}.bad{background:#3a1b1b;color:#f0aaaa}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:20px 0}.card,.panel{background:#111823;border:1px solid #202b3a;border-radius:15px}.card{padding:16px}.card span{display:block;color:#8ea0b5;font-size:12px;margin-bottom:7px}.card strong{font-size:17px}.panel{padding:18px;margin-top:14px}.panel h2{font-size:17px;margin:0 0 13px}.actions{display:flex;gap:9px;flex-wrap:wrap}button{border:1px solid #334155;background:#182231;color:#eef2f7;padding:9px 13px;border-radius:9px;font-weight:650;cursor:pointer}button.active{background:#17412a;border-color:#2e6c49;color:#aaf0bf}button:hover{background:#213047}.msg{color:#9fb0c3;font-size:13px;margin-top:12px}.client{display:flex;justify-content:space-between;gap:14px;align-items:center;padding:13px 0;border-top:1px solid #202b3a}.client:first-child{border-top:0}.meta{display:flex;flex-direction:column;gap:3px}.meta small{color:#8ea0b5}.policy{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.pill{padding:4px 8px;border-radius:999px;font-size:11px;background:#1a2230}.pill.on{background:#12351f;color:#9be7b0}.pill.block{background:#3b2e12;color:#f4d27a}@media(max-width:720px){.grid{grid-template-columns:repeat(2,1fr)}.client{align-items:flex-start;flex-direction:column}}</style></head><body><div class="wrap"><header><div><div class="sub">ZTE U60 Pro</div><h1>Proxy Mode</h1><div class="sub">Per-device transparent proxy & leak protection</div></div><span id="state" class="badge">unknown</span></header>
...
async function post(p){let t=tok();if(!t)throw Error('token required');let r=await fetch(p,{method:'POST',headers:{'X-Proxy-Token':t}});if(r.status===403)localStorage.removeItem('u60ProxyToken');let j=await r.json().catch(()=>({}));if(!r.ok)throw Error(j.error||('HTTP '+r.status));return j}
...
document.querySelectorAll('[data-t]').forEach(b=>b.onclick=()=>traffic(b.dataset.t));$('refresh').onclick=refresh;refresh();setInterval(refresh,15000);
</script></body></html>`
